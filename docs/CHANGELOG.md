...
- Added two optional, user-supplied **BRIA RMBG-2.0** modes to `Remove Background`: `Remove Background (BRIA Balanced)` (`model_fp16.onnx`, ~500 MB) and `Remove Background (BRIA High Quality)` (`model.onnx`, ~1 GB), selectable via `--rmbg-model bria-balanced|bria-high-quality` and Explorer context menus. FrameShift **never downloads, bundles, mirrors, hosts or redistributes** these models — they are obtained manually by the user from BRIA's official Hugging Face page (`https://huggingface.co/briaai/RMBG-2.0/tree/main`). Usage is non-commercial only (CC BY-NC 4.0); no commercial compatibility is claimed.
- Added a dedicated FrameShift notice dialog for the BRIA modes: when the model is missing, or when the installed file does not match the official BRIA checksum, FrameShift shows **Open BRIA page / Open folder / Re-check / Cancel** (plus **Use anyway** on checksum mismatch) instead of attempting any download. **Re-check** re-runs verification in place: once the user has placed the correct file it validates and proceeds straight to the action; otherwise the dialog stays open and reports the current status. SHA256 values are the content hashes of the official files, cross-checked against the exact official byte sizes (the HF tree-API `lfs.oid` is not the content hash for these Xet-backed files).
- Fixed the BRIA modes producing a uniformly translucent image instead of a cutout: the BRIA RMBG-2.0 transformers.js ONNX export already outputs a `[0,1]` alpha matte (output `alphas`, sigmoid baked into the graph), but the engine was applying `sigmoid` a second time (mapping background `0 → 0.5`, foreground `1 → 0.73` → alpha ~127–186 everywhere). Added an `OutputIsAlpha` per-model flag so BRIA mattes are used directly while BiRefNet (raw logits) keeps its sigmoid. Verified against the real model output (already in `[0,1]`).
- Added two optional installer components (`BRIA Remove Background (Balanced)` / `(High Quality)`), **unchecked by default**. When selected, the installer creates the target model folder plus `README.txt` and `LICENSE_NOTICE.txt` (no ONNX, no weights, no download) and adds the matching Explorer context-menu entry. When not selected, no BRIA folders, files or menu entries are created.
- Reorganized the installer components list so **FrameShift AI** appears first. Under `Remove Background`, only **Fast** is selected by default; **High Resolution (Matting)** and **High Resolution (General)** (both labelled "CPU only") and the two BRIA modes are now shown unchecked.
- Expanded `Remove Background` into a small local model catalog: `Fast` keeps the historical BiRefNet Lite FP16 behavior, while `High Resolution (Matting)` and `High Resolution (General)` download their own BiRefNet HR ONNX models on demand with dedicated SHA256 verification, catalog entries, and preflight flow.
- Added `--rmbg-model fast|high-resolution|high-resolution-general` to the shared CLI parser and wired the same option through Explorer context menus, so the three `Remove Background` variants call the same action with explicit model selection.
- Marked both high-resolution BiRefNet models as CPU-only in FrameShift `1.0.11` after validation on a high-end DirectML machine showed repeated GPU runtime memory failures during inference; `Fast` remains `DirectML first / CPU fallback`.
- Smoothed `Remove Background` progress reporting during ONNX inference so long CPU runs now advance steadily instead of appearing frozen on one percentage for most of the operation.
- Fixed `Remove Background` batch queue re-entry: launching the action again while the shared progress window is already open now reliably appends a new queue item to that window, including repeated launches of the exact same source file path. The queue no longer drops duplicate-path requests because each invocation now keeps its own in-memory queue identity.
- Fixed queued items inheriting the first launch's options: when a second launch was injected into an existing progress window, the inter-process queue carried only the file path, so the new item silently reused the first session's model (e.g. a `Remove Background (BRIA High Quality)` launch was processed with the already-running `Fast` model). The batch queue protocol now transmits each launch's CLI options (such as `--rmbg-model`) alongside the path as a per-line JSON message (with backward-compatible fallback for a bare path line); options are carried per queue item and merged over the session's shared options at execution time (the item wins), and the model preflight now runs in the queuing instance with that launch's real options. Two successive `Remove Background` launches with different models therefore stay distinct in the queue and each runs its own model.
- Documented the completed DPI/UI stabilization work across the active WinForms UI: added an official DPI audit reference, expanded the UI standardization rules for DPI-safe windows, `MinimumSize`, long labels, `TableLayoutPanel` usage, and shared geometry helpers, and refreshed the code index to reflect the active shared UI scaling responsibilities.
- Recorded the global Windows scaling fixes delivered by the UI pass for `125 %`, `133 %`, `150 %`, and `175 %`, including the removal of truncated text, overlapping controls, undersized groups, and layout instability on the validated screens.
