...
**Separate Audio.** `AudioChunkReader` charge tout le flux 44,1 kHz stéréo dans un `List<float>` puis le duplique avec `ToArray` (`src/FrameShift/Core/AI/SeparateAudio/AudioChunkReader.cs:23-29`, `50-79`). Sa construction à `AudioSeparationEngine.cs:64` n'accepte aucun token ; l'annulation ne reprend qu'après. Une heure représente environ 1,27 Go par tableau, donc au moins 2,54 Go au pic avant les tenseurs et buffers.

**Remove Noise.** `RemoveNoiseEngine` conserve l'audio, une copie d'analyse, spectres analysés réel/imaginaire, spectres traités, features, résultats ONNX, synthèse et sortie (`RemoveNoiseEngine.cs:63-100`, `195-266`). Pour une heure mono 48 kHz, les seules grandes représentations dépassent plusieurs gigaoctets et rendent pagination/OOM crédibles.

**Fallbacks vidéo.** RIFE et Upscale extraient des BMP sous `%TEMP%` (`RifeInterpolateVideoAction.cs:112-124`, `188-239`; `UpscaleVideoAction.cs:210-258`) sans estimer l'espace libre. RIFE conserve chaque passe (`RifeFrameInterpolationEngine.cs:139-189`). Dix minutes 1080p/30 fps représentent environ 112 Go pour une génération BMP ; x4 conserve l'entrée et deux générations croissantes. Le pipeline raw limite ce risque en mode nominal x2, mais x4, le mode explicite BMP et les fallbacks restent exposés.

**Correction ciblée.** Mesures immédiates : limites de durée/taille explicites et messages clairs, estimation de l'espace libre avant extraction, suppression des passes devenues inutiles. Ensuite seulement, segmenter Remove Noise et faire lire Separate Audio en flux/fenêtre glissante.

**État Separate Audio — corrigé, chantier streaming clôturé.** `AudioChunkReader` lit désormais le flux normalisé séquentiellement dans une unique fenêtre `LEN`, réutilisée d'un chunk à l'autre ; il ne conserve que l'overlap Demucs et une frame de lookahead pour détecter l'EOF exact. Sa mémoire propre est donc bornée et indépendante de la durée du fichier, et la lecture vérifie l'annulation entre les appels au provider. Les buffers Demucs/ONNX/OLA restent volontairement importants, mais sont eux aussi bornés par `LEN` et par la session active. Le temps de traitement et l'espace disque des stems WAV restent naturellement linéaires avec la durée ; aucune optimisation supplémentaire n'est nécessaire actuellement sur Separate Audio. Les validations DirectML et CPU ont conservé les mêmes durées, formats et tailles de stems ; l'écart CPU/DirectML se limite à de faibles arrondis PCM liés aux fournisseurs ONNX.
