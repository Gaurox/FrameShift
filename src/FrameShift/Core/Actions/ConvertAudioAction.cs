...
public sealed class ConvertAudioAction : IFrameShiftAction
{
    private readonly FfmpegRunner _ffmpegRunner;
    private readonly FfprobeRunner _ffprobeRunner;
    private readonly ToolLocator _toolLocator;

    public ConvertAudioAction(FfmpegRunner ffmpegRunner, FfprobeRunner ffprobeRunner, ToolLocator toolLocator)
    {
    ...
    public ActionDescriptor Descriptor { get; } = new(
        "convert-audio",
        "Convert Audio",
        "Converts an audio file to another container or codec profile.");
    ...
    public async Task<ActionExecutionResult> ExecuteAsync(ActionRequest request, CancellationToken cancellationToken)
    {
        ...
        var targetId = ConversionActionHelper.GetOption(request, ActionOptionKeys.Target, "mp3").ToLowerInvariant();
        if (!AudioConversionCatalog.IsSupportedTarget(targetId))
        {
        ...
        var target = AudioConversionCatalog.GetTargetById(targetId);
        var profile = AudioConversionCatalog.GetProfileById(profileId);
        var plan = AudioConversionCatalog.GetEncodingPlan(target.Id, profile.Id);
        ...
        request.ProgressReporter?.ReportState("processing", $"Preparing {profile.DisplayName.ToLowerInvariant()} conversion to {target.DisplayName.ToLowerInvariant()} on CPU...");
